...
const kMinMultiPartSize = 5 * 1024 * 1024
const kMaxParts = 10000                 // Maximum number of parts we will use
const kMaxBufferSize = 10 * 1024 * 1024 // Let's not get too crazy with buffers
const kSegmentSizeLimit = kMaxParts * kMaxBufferSize
...
func (this *ProtectedEntity) uploadSegment(ctx context.Context, baseName string, part int, startOffset int64, maxSegmentSize int64, reader io.Reader) (int64, error) {
	if maxSegmentSize > kSegmentSizeLimit {
		return 0, errors.Errorf("maxSegmentSize %d too large (%d is kSegmentSizeLimit)", maxSegmentSize, kSegmentSizeLimit)
	}
	name := segmentName(baseName, part, startOffset)
	awsBucketName := aws.String(this.rpetm.bucket)
	awsKey := aws.String(name)
	completedParts := make([]*s3.Part, kMaxParts)
	...
	var s3PartSize int64 = maxSegmentSize / kMaxParts
	if s3PartSize < kMinMultiPartSize {
		s3PartSize = kMinMultiPartSize
	}
	...
	for moreBits {
		...
		if uploadPart {
			...
			if partNumber == 0 && bytesRead < kMinMultiPartSize {
...
func (this *s3SegmentReader) getReaderForOffset(offset int64) (io.ReadCloser, error) {
	for _, curSegment := range this.segments {
		if curSegment.startOffset <= offset && curSegment.startOffset + curSegment.length > offset {
			s3Object, err := this.s3.GetObject(&s3.GetObjectInput{
				Bucket: aws.String(this.bucket),
				Key:    aws.String(curSegment.key),
			})
